...
class GeniAIExpection(Exception):
    """GenAI Exception base class"""
    
class ChatBot:
    """Chat can only have one candidate type"""
    CHATBOT_NAME = 'Oculus'
    ...
    def preload_conversation(self, conversation_history=None):
        if isinstance(conversation_history, list):
            ...
            self._conversation_history = [
                ...
                self._construct_message('{"text": "Sure, I can return the object as a regular JSON object with the key as `text`. Here is an example {"text": "Your Output"}."}', 'model')
            ]
